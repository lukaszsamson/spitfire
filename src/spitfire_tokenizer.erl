...
tokenize(String, Line, Column, #spitfire_tokenizer{} = Scope) ->
  ...
  Scope =
    lists:foldl(fun
      ...
      ({static_atoms_encoder, StaticAtomsEncoder}, Acc) when is_function(StaticAtomsEncoder) ->
        Acc#spitfire_tokenizer{static_atoms_encoder=StaticAtomsEncoder};
      ({preserve_comments, PreserveComments}, Acc) when is_function(PreserveComments) ->
        Acc#spitfire_tokenizer{preserve_comments=PreserveComments};
      ({unescape, Unescape}, Acc) when is_boolean(Unescape) ->
        Acc#spitfire_tokenizer{unescape=Unescape};
      (_, Acc) ->
        Acc
    end, #spitfire_tokenizer{identifier_tokenizer=IdentifierTokenizer, column=Column}, Opts),
...
tokenize([$?, $\\, H | T], Line, Column, Scope, Tokens) ->
  Char = spitfire_interpolation:unescape_map(H),
  ...
  case spitfire_interpolation:extract(Line, Column + 2, Scope, true, T, H) of
    {NewLine, NewColumn, Parts, Rest, InterScope} ->
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        ...
        {ok, Unescaped} ->
          Key = case Scope#spitfire_tokenizer.existing_atoms_only of
            true  -> atom_safe;
            false -> atom_unsafe
          end,
...
tokenize([$: | String] = Original, Line, Column, Scope, Tokens) ->
  case tokenize_identifier(String, Line, Column, Scope, false) of
    ...
    empty when Scope#spitfire_tokenizer.cursor_completion == false ->
  ...
  case tokenize_number(T, [H], 1, false) of
    ...
    {[I | Rest], Number, Original, _Length} when ?is_upcase(I); ?is_downcase(I); I == $_ ->
      if
        ...
        Scope#spitfire_tokenizer.cursor_completion /= false ->
          tokenize([], Line, Column, Scope, Tokens);
...
tokenize(String, Line, Column, OriginalScope, Tokens) ->
  case tokenize_identifier(String, Line, Column, OriginalScope, not previous_was_dot(Tokens)) of
    ...
    empty when OriginalScope#spitfire_tokenizer.cursor_completion == false ->
...
tokenize_eol(Rest, Line, Scope, Tokens) ->
  {StrippedRest, Column} = strip_horizontal_space(Rest, Scope#spitfire_tokenizer.column),
  IndentedScope = Scope#spitfire_tokenizer{indentation=Column-1},
  ...
  case strip_horizontal_space(T, 0) of
    {[$# | R], _} ->
      case tokenize_comment(R, [$#]) of
        ...
        {Rest, Comment} ->
          preserve_comments(Line, Column, Tokens, Comment, Rest, Scope),
          tokenize_dot(Rest, Line, Scope#spitfire_tokenizer.column, DotInfo, Scope, Tokens)
      end;
    {"\r\n" ++ Rest, _} ->
      tokenize_dot(Rest, Line + 1, Scope#spitfire_tokenizer.column, DotInfo, Scope, Tokens);
    {"\n" ++ Rest, _} ->
      tokenize_dot(Rest, Line + 1, Scope#spitfire_tokenizer.column, DotInfo, Scope, Tokens);
    {Rest, Length} ->
...
handle_strings(T, Line, Column, H, Scope, Tokens) ->
  case spitfire_interpolation:extract(Line, Column, Scope, true, T, H) of
    {error, Reason} ->
...
is_unnecessary_quote([Part], Scope) when is_list(Part) ->
  case (Scope#spitfire_tokenizer.identifier_tokenizer):tokenize(Part) of
...
unsafe_to_atom(Part, Line, Column, #spitfire_tokenizer{}) when
...
unsafe_to_atom(Part, Line, Column, #spitfire_tokenizer{static_atoms_encoder=StaticAtomsEncoder}) when
...
unsafe_to_atom(Binary, Line, Column, #spitfire_tokenizer{existing_atoms_only=true}) when is_binary(Binary) ->
  try
...
unsafe_to_atom(Binary, _Line, _Column, #spitfire_tokenizer{}) when is_binary(Binary) ->
  {ok, binary_to_atom(Binary, utf8)};
unsafe_to_atom(List, Line, Column, #spitfire_tokenizer{existing_atoms_only=true}) when is_list(List) ->
  try
    {ok, list_to_existing_atom(List)}
  catch
    error:badarg -> {error, {?LOC(Line, Column), "unsafe atom does not exist: ", List}}
  end;
unsafe_to_atom(List, _Line, _Column, #spitfire_tokenizer{}) when is_list(List) ->
...
extract_heredoc_with_interpolation(Line, Column, Scope, Interpol, T, H) ->
  case extract_heredoc_header(T) of
    {ok, Headerless} ->
      ...
      case spitfire_interpolation:extract(Line, Column, Scope, Interpol, [$\n|Headerless], [H,H,H]) of
...
unescape_tokens(Tokens, Line, Column, #spitfire_tokenizer{unescape=true}) ->
  case spitfire_interpolation:unescape_tokens(Tokens) of
    {ok, Result} ->
      {ok, Result};
...
unescape_tokens(Tokens, _Line, _Column, #spitfire_tokenizer{unescape=false}) ->
...
preserve_comments(Line, Column, Tokens, Comment, Rest, Scope) ->
  case Scope#spitfire_tokenizer.preserve_comments of
...
tokenize_identifier(String, Line, Column, Scope, MaybeKeyword) ->
  case (Scope#spitfire_tokenizer.identifier_tokenizer):tokenize(String) of
...
suggest_simpler_unexpected_token_in_error(Wrong, Line, WrongColumn, Scope) ->
  ...
  case (Scope#spitfire_tokenizer.identifier_tokenizer):tokenize(NFKC) of
    {error, _Reason} ->
       ConfusableSkeleton = 'Elixir.String.Tokenizer.Security':confusable_skeleton(Wrong),
       case (Scope#spitfire_tokenizer.identifier_tokenizer):tokenize(ConfusableSkeleton) of
         {_, Simpler, _, _, _, _} ->
...
handle_terminator(Rest, Line, Column, #spitfire_tokenizer{terminators=none} = Scope, Token, Tokens) ->
  tokenize(Rest, Line, Column, Scope, [Token | Tokens]);
handle_terminator(Rest, Line, Column, Scope, Token, Tokens) ->
  #spitfire_tokenizer{terminators=Terminators} = Scope,
...
check_terminator({Start, Meta}, Terminators, Scope)
  ...
  Indentation = Scope#spitfire_tokenizer.indentation,
  {ok, Scope#spitfire_tokenizer{terminators=[{Start, Meta, Indentation} | Terminators]}};
  ...
  NewScope =
    case Terminators of
      ...
      [{Start, _, PreviousIndentation} = Previous | _] when Indentation =< PreviousIndentation ->
        Scope#spitfire_tokenizer{mismatch_hints=[Previous | Scope#spitfire_tokenizer.mismatch_hints]};

      _ ->
        Scope
    end,

  {ok, NewScope#spitfire_tokenizer{terminators=[{Start, Meta, Indentation} | Terminators]}};
...
check_terminator({'end', {EndLine, _, _}}, [{'do', _, Indentation} | Terminators], Scope) ->
  NewScope =
    ...
    case Scope#spitfire_tokenizer.indentation > Indentation of
      true ->
        Hint = {'end', EndLine, Scope#spitfire_tokenizer.indentation},
        Scope#spitfire_tokenizer{mismatch_hints=[Hint | Scope#spitfire_tokenizer.mismatch_hints]};

      false ->
        Scope
    end,

  {ok, NewScope#spitfire_tokenizer{terminators=Terminators}};
...
check_terminator({End, {EndLine, EndColumn, _}}, [{Start, {StartLine, StartColumn, _}, _} | Terminators], Scope)
  ...
  case terminator(Start) of
    End ->
      {ok, Scope#spitfire_tokenizer{terminators=Terminators}};
...
missing_terminator_hint(Start, End, #spitfire_tokenizer{mismatch_hints=Hints}) ->
...
tokenize_sigil_contents([H | T] = Original, [S | _] = SigilName, Line, Column, Scope, Tokens)
    when ?is_sigil(H) ->
  case spitfire_interpolation:extract(Line, Column + 1, Scope, ?is_downcase(S), T, sigil_terminator(H)) of
...
track_ascii(false, Scope) -> Scope#spitfire_tokenizer{ascii_identifiers_only=false}.
...
error(Reason, Rest, #spitfire_tokenizer{warnings=Warnings}, Tokens) ->

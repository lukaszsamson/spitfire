defmodule Spitfire.Property.Generators do
  ...
  def expr(context, depth, interp_depth, block_depth) do
    base_generators = [
      ...
      {3, quoted_atom()},
      {4, variable()},
      {4, string_like(:binary, depth, interp_depth, block_depth)},
  ...
  defp string_like(:binary, depth, interp_depth, block_depth) do
    string_frag(depth, interp_depth, block_depth, "\"", "\"")
  end

  defp string_like(:charlist, depth, interp_depth, block_depth) do
    string_frag(depth, interp_depth, block_depth, "'", "'")
  end

  defp string_frag(depth, interp_depth, block_depth, opener, closer) when interp_depth > 0 do
    fragments =
      one_of([
        constant(""),
  ...
  defp string_frag(_depth, _interp_depth, _block_depth, opener, closer) do
    string(:alphanumeric, length: 0..6)
    |> map(&"#{opener}#{&1}#{closer}")
  end

  defp heredoc(kind, depth, interp_depth, block_depth) do
  ...
  defp quote_block(depth, interp_depth, block_depth) do
    bind(expr(:expr, depth, interp_depth, block_depth), fn body ->
      one_of([
        ...
        constant("quote do\n  #{body}\nend"),
        unquote_form(depth, interp_depth, block_depth, body)
      ])
    end)
  end
  ...
  defp range_expr(depth, interp_depth, block_depth) do
    bind(
      {expr(:expr, depth, interp_depth, block_depth),
       expr(:expr, depth, interp_depth, block_depth), one_of([constant(nil), integer(-3..3)])},
      fn
        {left, right, nil} ->
          constant("#{left}..#{right}")

        {left, right, step} ->
          constant("#{left}..#{right}//#{step}")
      end
    )
  end

  defp pipe_expr(depth, interp_depth, block_depth) do
    bind(
      {expr(:expr, depth, interp_depth, block_depth),
       expr(:expr, depth, interp_depth, block_depth)},
      fn {left, right} ->
  ...
  defp concat_expr(depth, interp_depth, block_depth) do
    bind(
      {string_like(:binary, depth, interp_depth, block_depth),
       string_like(:binary, depth, interp_depth, block_depth)},
      fn {left, right} ->
        constant("#{left} <> #{right}")
      end
  ...
  defp module_attribute(depth, interp_depth, block_depth) do
  ...
  defp unquote_form(depth, interp_depth, block_depth, inner) do
    bind(expr(:expr, max(depth - 1, 0), interp_depth, block_depth), fn argument ->
      one_of([
        constant("quote do: unquote(#{argument})"),
        constant("quote do: unquote_splicing(#{argument})"),
        constant("quote do\n  unquote(#{argument})\n  #{inner}\nend")
      ])
    end)
  end

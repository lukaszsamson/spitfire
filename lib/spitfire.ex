defmodule Spitfire do
  ...
  defp parse_capture_expression(parser) do
    trace "parse_capture_expression", trace_meta(parser) do
      ...
      {rhs, parser} =
        if peek_token_type(parser) == :when_op and capture_has_do_block_args?(rhs) do
          parse_infix_expression(next_token(parser), rhs)
        else
          {rhs, parser}
        end
  ...
  defp capture_has_do_block_args?({_, _, args}) when is_list(args) do
  ...
  defp normalize_fn_clauses([lhs, {:->, meta, [[lhs], rhs]} | rest]) do
    [{:->, meta, [[lhs], rhs]} | rest]
  end
